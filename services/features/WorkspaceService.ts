...
export default class WorkspaceService
{
     ...
     */
    public static async createWorkspace(title: string, organizationId: string) : Promise<Workspace>
    {
        // * Create a new workspace
        ...
        if (response.ok)
        {
            ...
            return new Workspace(data.id, data.title, data.ownedBy, data.contacts, data.projects);
        }
        else {
            // ! Throw an error
    ...
    public static async getWorkspaces() : Promise<Workspace[]>
    {
        ...
        if (response.ok)
        {
            ...
            return data.map((workspace: Workspace) => new Workspace(workspace.id, workspace.title, workspace.ownedBy, workspace.contacts, workspace.projects));
        }
        else {
            // ! Throw an error
    ...
    {
        // # Send the request to the API
        const response = await fetch(`https://localhost:7006/api/workspaces/${id}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
     ...
     */
    public static async updateWorkspace(id: string, title: string, contactsToAdd: string[], contactsToRemove: string[], projectsToAdd: string[], projectsToRemove: string[]) : Promise<Workspace>
    {
        // * Create a new workspace
        const workspace = new UpdateWorkspaceRequest(title, contactsToAdd, contactsToRemove, projectsToAdd, projectsToRemove);
     ...
     */
    public static async createWorkspaceResource(workspaceId: string, title: string, url: string) : Promise<Resource>
    {
        // * Create a new workspace resource
        const resource = new CreateResourceRequest(title, url);

        // # Send the request to the API
        const response = await fetch(`https://localhost:7006/api/workspaces/${workspaceId}/resources`, {
            method: 'POST',
        ...
        if (response.ok)
        {
            ...
            return new Resource(data.id, data.title, data.description, data.url, data.type);
        }
        else {
            // ! Throw an error
     ...
     */
    public static async getWorkspaceResources(workspaceId: string) : Promise<Resource[]>
    {
        // # Send the request to the API
        const response = await fetch(`https://localhost:7006/api/workspaces/${workspaceId}/resources`, {
            method: 'GET',
        ...
        if (response.ok)
        {
            ...
            return data.map((resource: Resource) => new Resource(resource.id, resource.title, resource.description, resource.url, resource.type));
        }
        else {
            // ! Throw an error
     ...
     */
    public static async getWorkspaceResource(workspaceId: string, resourceId: string) : Promise<Resource>
    {
        // # Send the request to the API
        const response = await fetch(`https://localhost:7006/api/workspaces/${workspaceId}/resources/${resourceId}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
     ...
     */
    public static async updateWorkspaceResource(workspaceId: string, resourceId: string, title: string, url: string, description: string, type: string) : Promise<Resource>
    {
        // * Create a new workspace resource
     ...
     * @param resourceId
     */
    public static async deleteWorkspaceResource(workspaceId: string, resourceId: string) : Promise<void>
    {
        // # Send the request to the API
        const response = await fetch(`https://localhost:7006/api/workspaces/${workspaceId}/resources/${resourceId}`, {
            method: 'DELETE',
